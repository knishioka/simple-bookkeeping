...
CREATE OR REPLACE FUNCTION public.user_role_in_organization(org_id UUID)
RETURNS user_role
LANGUAGE sql
SECURITY DEFINER
STABLE
...
BEGIN
    ...
    IF org_id IS NOT NULL THEN
        user_role_value := public.user_role_in_organization(org_id);
    ELSE
...
CREATE POLICY "Organizations: Only admins can update their organizations"
    ...
    USING (id = ANY(public.user_organization_ids()) AND public.has_role('admin'::user_role, id))
    WITH CHECK (id = ANY(public.user_organization_ids()) AND public.has_role('admin'::user_role, id));
...
CREATE POLICY "Users: Only admins can insert users"
    ON users FOR INSERT
    WITH CHECK (public.has_role('admin'::user_role, organization_id));
...
CREATE POLICY "Users: Admins can update users in their organization"
    ON users FOR UPDATE
    USING (organization_id = ANY(public.user_organization_ids()) AND public.has_role('admin'::user_role, organization_id))
    WITH CHECK (organization_id = ANY(public.user_organization_ids()) AND public.has_role('admin'::user_role, organization_id));

-- User organizations policies
CREATE POLICY "User orgs: Users can view their own relationships"
    ON user_organizations FOR SELECT
    USING (user_id = auth.uid() OR organization_id = ANY(public.user_organization_ids()));
...
CREATE POLICY "Periods: Accountants can insert periods"
    ...
    WITH CHECK (organization_id = ANY(public.user_organization_ids()) AND public.has_role('accountant'::user_role, organization_id));

CREATE POLICY "Periods: Accountants can update periods"
    ON accounting_periods FOR UPDATE
    USING (organization_id = ANY(public.user_organization_ids()) AND public.has_role('accountant'::user_role, organization_id))
    WITH CHECK (organization_id = ANY(public.user_organization_ids()) AND public.has_role('accountant'::user_role, organization_id));
...
CREATE POLICY "Accounts: Admins can delete accounts"
    ...
    USING (organization_id = ANY(public.user_organization_ids()) AND public.has_role('admin'::user_role, organization_id) AND is_system_account = false);
...
CREATE POLICY "Journal: Accountants can update draft entries"
    ...
    USING (
        ...
        AND public.has_role('accountant'::user_role, organization_id)
        AND status = 'draft'
    )
    WITH CHECK (
        organization_id = ANY(public.user_organization_ids()) 
...
CREATE POLICY "Journal: Admins can delete draft entries"
    ...
    USING (
        organization_id = ANY(public.user_organization_ids()) 
        AND public.has_role('admin'::user_role, organization_id)
        AND status = 'draft'
    );
...
CREATE POLICY "Lines: Users can view lines of accessible entries"
    ...
    USING (
        EXISTS (
            ...
            AND je.organization_id = ANY(public.user_organization_ids())
        )
    );
...
CREATE POLICY "Lines: Accountants can manage lines"
    ...
    WITH CHECK (
        EXISTS (
            ...
            WHERE je.id = journal_entry_lines.journal_entry_id
            AND je.organization_id = ANY(public.user_organization_ids())
            AND public.has_role('accountant'::user_role, je.organization_id)
            AND je.status = 'draft'
        )
    );
...
CREATE POLICY "Lines: Admins can delete lines of draft entries"
    ...
    USING (
        EXISTS (
            ...
            WHERE je.id = journal_entry_lines.journal_entry_id
            AND je.organization_id = ANY(public.user_organization_ids())
            AND public.has_role('admin'::user_role, je.organization_id)
            AND je.status = 'draft'
        )
    );
